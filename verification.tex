...
The \c{get} and \c{cas} operations are specified using \emph{logically atomic triples}, composed of (1) \emph{private} pre- and postconditions for the caller, as in Hoare triples, and (2) \emph{public} pre- and postconditions for the \emph{linearization point}.
...
\refTirName{cas-spec} says that $\c{cas}\ \zooLoc{}s\ \v{befores}\ \v{afters}$ performs an MCAS operation on locations $\zooLoc{}s$ with expected values \v{befores} and desired values \v{afters}; on success, it atomically updates $\zooLoc{}s$ from \v{befores} --- again, \emph{up to physical equality} --- to \v{afters}; on failure, it must have observed a location whose value was \emph{physically} different than expected.
